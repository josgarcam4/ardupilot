...
#        self.takeoffAndMoveAway()
    ...
    def GPSSpoofDetectorNominal(self, timeout=60):
        """
        Step 1 - Baseline: arm, takeoff, fly to waypoint 50 m North, loiter 20 s.
        Verify the detector stays NOMINAL (no false alerts) during clean flight.
        Note: Uses detector default parameters (AP_Param registration in progress)
        """
        self.progress("GPS Spoof Detector - Nominal baseline test")
        self.progress("Using detector defaults: GSPF_ENABLE=0, TH_LOW=0.6, TH_HIGH=1.0")

        # Step 1a: clear any residual GPS glitch / velocity error from previous runs
        self.set_parameters({
            "SIM_GPS1_GLTCH_X": 0,
            "SIM_GPS1_GLTCH_Y": 0,
            "SIM_GPS1_GLTCH_Z": 0,
        ...
        # Step 1c: arm and takeoff to 15 m in LOITER
        self.takeoff(15, mode="LOITER")
        self.progress("Reached hover altitude; loitering for 20 s without spoofing")

        # Step 1e: loiter 20 s — no spoofing → NOMINAL must hold
        self.delay_sim_time(20)

        # Detector must NOT have emitted any GSPF warning during clean flight.
        # wait_statustext with timeout=1 is used as a negative check:
        # if the text arrives it raises, so we catch and pass.
        received_warning = False
        try:
            self.wait_statustext("GSPF:", timeout=1)
            received_warning = True
        except Exception:
            pass
        if received_warning:
            raise NotAchievedException("GSPF alert fired during clean flight — false alarm")

        self.land_and_disarm()
        self.progress("GPS Spoof Detector Nominal Test: PASSED — no false alarms")

    def GPSSpoofGradualLoiter(self, timeout=120):
        """
        Full end-to-end spoofing experiment:

        Phase 1 - Nominal flight (arm, takeoff, fly to waypoint).
        Phase 2 - Inject gradual GPS velocity drift via SIM_GPS1_VERR (Feature 1)
                  and simultaneous position glitch ramp via SIM_GPS1_GLTCH (Feature 4).
        Phase 3 - Verify SUSPICIOUS notification arrives within 15 s.
        Phase 4 - Sustain attack; verify CONFIRMED notification within 60 s.
        Phase 5 - Remove spoofing; verify system lands cleanly.

        Spoofing model used:
          - SIM_GPS1_VERR_{X,Y}: linear ramp of velocity error in NED (m/s).
            This creates a persistent v_GPS - v_EKF residual that drives the
            CUSUM accumulator in compute_feature_vel_divergence().
          - SIM_GPS1_GLTCH_{X,Y}: simultaneous position glitch (degrees).
            Causes hAcc/sAcc inconsistency that drives Feature 4.
        """
        self.progress("GPS Spoof Gradual Loiter - full detection experiment")

        # --- Phase 1: configure and take off -----------------------------------
        # Note: GSPF detector parameters will use defaults (AP_Param registration in progress)
        # - GSPF_ENABLE: 0 (default - disabled, but detector still runs for this test)
        # - GSPF_TH_LOW: 0.6 (default)
        # - GSPF_TH_HIGH: 1.0 (default)
        # - GSPF_INNOV_TH: 5.0 (default)
        # - GSPF_CUSUM_K: 0.5 (default)
        # - Weights: W1=0.35, W2=0.30, W3=0.20, W4=0.15 (defaults)
        self.progress("Using detector defaults - tuning via parameters pending AP_Param fix")

        # Ensure no residual glitch from a previous test run
        self.set_parameters({
            "SIM_GPS1_GLTCH_X": 0,
            "SIM_GPS1_GLTCH_Y": 0,
            "SIM_GPS1_GLTCH_Z": 0,
        ...
        self.takeoff(15, mode="LOITER")
        self.progress("Phase 1 complete - hovering in LOITER, EKF steady")

        # Brief stabilisation pause before injecting attack
        self.delay_sim_time(5)

        # --- Phase 2: inject spoofing attack and verify detection ----------------------------------
        # Inject GPS velocity error (m/s) and position glitch (degrees).
        # At 10 Hz the CUSUM accumulates: each tick adds (vel_err/sAcc - k).
        # With sAcc~0.3 (default ACC param) and k=0.5:
        #   nu ~ vel_err/0.3 - 0.5; at vel_err=1.5 m/s -> nu=4.5/tick
        #   CUSUM reaches innov_thresh=5 in ~2 ticks -> f1_score > 0.9 quickly.
        # Score = 0.35*f1 + 0.25*f4 + ... should exceed TH_LOW=0.6 after ~5 ticks (0.5 s).
        #
        # Position glitch of 9e-5 degrees ~= 1 m at the equator,
        # causing the accuracy-consistency feature (F4) to accumulate as well.
        self.progress("Phase 2 - injecting static spoofing attack")
        self.set_parameters({
            "SIM_GPS1_VERR_X": 1.5,        # North velocity error (m/s) - medium attack
            "SIM_GPS1_VERR_Y": 0.75,       # East  velocity error (m/s)
            "SIM_GPS1_GLTCH_X": 0.00009,   # lat glitch (degrees) - ~1m at equator
            "SIM_GPS1_GLTCH_Y": 0.000045,  # lon glitch (degrees)
        })

        self.progress("Phase 2 - waiting for SUSPICIOUS detection (within 15s of attack)")
        self.wait_statustext("GSPF: SUSPICIOUS", timeout=15)
        self.progress("Phase 3 PASSED - SUSPICIOUS state detected and notified")

        # --- Phase 4: sustain attack to reach CONFIRMED state --------
        # SUSPICIOUS -> CONFIRMED requires 30 consecutive ticks (3 s) with
        # score > TH_HIGH=1.0.  Keep the attack running.
        # Notification is throttled to 2 s in CONFIRMED state.
        self.progress("Phase 4 - sustaining attack for CONFIRMED detection (within 60s)")
        self.wait_statustext("GSPF: CONFIRMED SPOOFING", timeout=60)
        self.progress("Phase 4 PASSED - CONFIRMED state detected and notified")

        # --- Phase 5: remove spoofing and land ---------------------------------
        self.progress("Phase 5 - removing spoofing and landing")
        self.set_parameters({
            "SIM_GPS1_GLTCH_X": 0,
            "SIM_GPS1_GLTCH_Y": 0,
            "SIM_GPS1_GLTCH_Z": 0,
        ...
        # Allow EKF to re-converge before landing
        self.delay_sim_time(5)
        self.land_and_disarm()
        self.progress("GPS Spoof Gradual Loiter Test: PASSED - all phases verified")
...
#        self.delay_sim_time(1)
    ...
    def tests1d(self):
        ...
        ret = ([
             ...
             self.GPSGlitchAuto,
             self.GPSSpoofDetectorNominal,
             self.GPSSpoofGradualLoiter,
             self.GPSFixTypes,
             self.ModeAltHold,
             self.ModeLoiter,
             self.SimpleMode,
