#!/usr/bin/env python3
"""
Manual testing script for AP_GpsSpoofDetect
Launches SITL, arms, takes off, injects spoofing, and monitors detector
"""
...
def test_detector():
    # Connect to SITL
    print("Connecting to SITL on 127.0.0.1:14550...")
    mav = mavutil.mavlink_connection('127.0.0.1:14550', baud=115200, timeout=30)

    # Wait for heartbeat
    print("Waiting for heartbeat...")
    mav.wait_heartbeat(timeout=10)
    print(f"Connected to {mav.sysid}")
    time.sleep(1)
    ...
    mav.mav.set_mode_send(mav.target_system, 1, 5)  # base_mode=1 (custom), custom_mode=5 (LOITER)
    time.sleep(2)
    ...
    while time.time() - start_time < 60:
        ...
        if msg:
            ...
            if "GSPF" in text:
                gspf_messages.append((time.time()-start_time, text))
        time.sleep(0.1)
...
if __name__ == '__main__':
    ...
    sitl_log = open('/tmp/sitl_manual_test.log', 'w')
